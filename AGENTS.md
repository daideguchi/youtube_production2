...
## 0) 最初に読む（必須）
- 入口: `START_HERE.md`
- 確定フロー（正本）: `ssot/OPS_CONFIRMED_PIPELINE_FLOW.md`
- 確定ロジック（最終ルール）: `ssot/【消さないで！人間用】確定ロジック`
- 入口索引（何を叩くか）: `ssot/OPS_ENTRYPOINTS_INDEX.md`
- ログ配置（正本）: `ssot/OPS_LOGGING_MAP.md`
- 生成物の保持/削除（正本）: `ssot/PLAN_OPS_ARTIFACT_LIFECYCLE.md`
- エージェント運用（本書の詳細）: `ssot/OPS_AGENT_PLAYBOOK.md`
...
## 2) ゴミ削除の基本方針（強制）
- “確実ゴミ”のみ削除可（基準: `ssot/PLAN_LEGACY_AND_TRASH_CLASSIFICATION.md`）。
- tracked な削除は **archive-first**（`backups/graveyard/`）→削除→ `ssot/OPS_CLEANUP_EXECUTION_LOG.md` に記録。
