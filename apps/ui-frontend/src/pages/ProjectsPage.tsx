...
useEffect(() => {
  ...
  const spreadsheetHeaders = spreadsheetData?.headers ?? [];
  ...
  const creationFlagIndex = headerIndexMap[HEADER_LABELS.creationFlag] ?? -1;
  const progressIndex = headerIndexMap[HEADER_LABELS.progress] ?? -1;

  const tableRows = useMemo(() => {
    if (!selectedChannel || !spreadsheetData) {
      return [] as SpreadsheetRow[];
    }
    const videoIdx = headerIndexMap[HEADER_LABELS.videoNumber] ?? -1;
    const scriptIdIdx = headerIndexMap[HEADER_LABELS.scriptId] ?? -1;
    const mapped = spreadsheetData.rows.map((cellsRaw, idx) => {
      const normalizedCells = spreadsheetHeaders.map((_, colIdx) => {
        const value = cellsRaw[colIdx];
  ...
  }, [selectedChannel, spreadsheetData, spreadsheetHeaders, headerIndexMap]);
    ...
    try {
      const payload = {
        ...
        config: {
          ...
          llm_model: formValues.llmModel || DEFAULT_FORM.llmModel,
                      ...
                      return (
                        ...
                        >
                          ...
                          {row.cells.map((cell, colIdx) => {
                            const header = spreadsheetHeaders[colIdx] ?? `col-${colIdx}`;
