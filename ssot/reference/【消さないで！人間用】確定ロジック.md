...
- **ポーズ挿入の区切り記号は `---` のみ**（1行単独）。`***` / `___` / `///` 等は使わない（TTSが不自然に途切れる原因）。
  ...
  - 非・睡眠チャンネルに **睡眠誘導フレーミング（寝落ち/睡眠用/安眠/おやすみ締め/布団/ベッド/枕/就寝/眠り等）** が混入していないこと（混入は `script_validation` で不合格）
  - **有料LLM/モデル/プロバイダの無断利用は禁止**（=ユーザーの課金枠/予算を勝手に消費する行為）
    - クレジット無しの台本生成は **qwen（`--auth-type qwen-oauth` 固定）**
    - クレジット有りでAPIを使う場合の既定は **DeepSeek（`script-main-1`）**（別モデルへ勝手に切替しない）
...
- 読みLLM（auditor/reading resolution）は **デフォルト無効（推論=対話型AIエージェント / 読みLLM無効）**。`SKIP_TTS_READING=1`（lockdown中は `0` 禁止。例外は `YTM_EMERGENCY_OVERRIDE=1` を明示した実行のみ）。
- **誤読防止（グローバル確定ルール）**:
...
### 0) 最重要（Hot=未投稿の資産ルール / Storage）
- Hot（未投稿 / Unposted）= **まだ投稿していない**回の資産。ここを絶対に失わない。
- **非交渉**: Hot（未投稿）の CapCutドラフト/Workset/編集中素材は **必ず Mac ローカルに実体**があること（「Lenovo/NASにしか無い」はNG）。
- 外部ストレージ（Lenovo/NAS）が不安定でも **Macの生成/編集を止めない**（共有への同期は復旧後に後追いでOK）。
- 参照パスは **必ず実体へ**（壊れsymlink/存在しないパスを残さない）。CapCut参照パス事故はP0。
- 自動化で未投稿を消さない（Never auto-delete unposted）:
  - purge/cleanup は **投稿済み（Published）** だけ検討対象。未投稿は削除/破壊/外部only化を禁止（Freezeも同様）。
- 直感的チェック（UI/CLI）:
  - UI（このMacの状態）: `/agent-board`（申し送り/進捗の板）を最初に確認。
  - CLI: `./ops storage doctor`（root配線の健全性） / `python3 scripts/ops/hot_assets_doctor.py --all-channels`（Hot資産P0チェック）
  - SSD節約（重複回避）: `workspaces/video/input/**` に wav/srt を“コピー”し続けると容量を食うため、wav は `workspaces/audio/final/**` を参照する **symlink** を推奨（SRTは小さいのでコピーでもOK）。
    - 入口（dry-run→run）: `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.sync_audio_inputs --dry-run --wav-policy symlink --wav-dedupe --hash-wav`
- 参照の正本: `ssot/ops/OPS_HOTSET_POLICY.md` / `ssot/ops/OPS_CAPCUT_DRAFT_STORAGE_STRATEGY.md` / `ssot/ops/OPS_SHARED_ASSET_STORE.md`
  - 保存先/配線の1枚絵（正本）: `ssot/ops/OPS_IMAGE_DDD_STORAGE_MAP_AND_APPROVAL.md`（日本語メイン＋英語併記）
  - 共有の実体（Lenovo内蔵SSD vs 外付け）: **内蔵SSD優先**、外付けはCold/重い領域だけ（例: uploads/archive/video） (Internal SSD preferred; external only for cold/bulk)
...
- 内部処理の順序（auto_capcut_run.py）
  ...
  2) ベルト生成: `belt_mode` 既定は `llm`（image_cues からLLM生成）。`existing` を指定すると run_dir 内の belt_config.json をそのまま使い、なければスキップ。`equal`/`grouped` は labels や chapters/episode_info.json が必要。**ただし preset の `belt.enabled=false` のCHはベルト生成自体をスキップ**する。  
  ...
  - 主要設定ポイント
    ...
    - **画像の意味整合（最重要）**: 各cueの画像は **「そのセクション（区切ったキュー）の内容を正確に表現する」** こと。**絶対禁止**: キーワード辞書・固定モチーフ・固定プール等で主題を機械置換/自動決め打ちする（例: 「時間」→時計/懐中時計）。抽象・比喩でも、当該セクションの具体的な行動/物体/状況に必ず紐づけ、同じ象徴を連発しない。曖昧なら `visual_cues_plan.json` を先に直す（pending→ready）。また **禁止例の具体列挙はプロンプト上のprimingになり得る** ため、実運用のプロンプトでは禁止カテゴリを一般化して書く（例: cliché symbols）。
	    - **FLUX schnell の類似画像対策（重要）**: `fireworks_flux_1_schnell_fp8` はプロンプト長が短く、長文テンプレ/ガードレールは内部で短縮されて **cue間の差分が潰れやすい**（=同じ構図/同じ場所の量産が起きる）。対策は 3 点:
	      ...
	      3) cue間の差分は **LLM/THINK の推論結果（`visual_cues_plan.json` の `refined_prompt` 等）**で担保する（勝手なローカルheuristicで決め打ちしない）。特に schnell は `refined_prompt` を必須にし、各cueの短い英語プロンプトに camera angle/distance・pose/action・setting/props・lighting を入れ、隣接で同じ構図/同じ場所にならないようにする。曖昧/類似が出たら `visual_cues_plan.json` を修正して再実行する。
		      - 補足（更新）: 動画内画像（`visual_image_gen`）は `nanobanana=batch` を既定にし、Gemini では **Batch 運用**をデフォルトにする（正本: `ssot/DECISIONS.md:D-016`）。schnell は必要時の明示選択へ（削除はしない）。
	    - **画像なしドラフト（注意）**: `--nanobanana none` は **外部画像生成を止める**ため、run_dir の `images/*.png` を **PLACEHOLDER** で埋める（CapCut上も白い疑似画像に見える＝挿入漏れではなく仕様）。後から画像を入れるなら `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.regenerate_images_from_cues --run <run_dir> --channel <CH> --overwrite --nanobanana batch`（即時に欲しければ `--nanobanana direct`）→ 必要に応じて `auto_capcut_run --belt-mode existing --resume` でドラフトを上書き再生成（ドラフト側の同期が崩れたら `sync_srt2images_materials --draft <draft_dir>`）。
    - **テンプレ本体の恒久クリーン**（定期）:
    ...
    - **CH09/CH26: 偉人情報（person_info_text）**: `portrait_left_subtitles_v1` のとき左下に偉人情報を出す。SoT は `ssot/reference/person_info_master.json`。run_dir の `episode_info.json` の `person` は以下を埋める:
      - `name`（表示名。例: `ジョン・フォン・ノイマン`）
      - `years`（例: `1903年-1957年` / `紀元前4年-65年`。不明/概念の場合は空でOK）
      - `occupation`（例: `数学者`。無い場合は互換で `achievement` を `occupation` とみなす）
      - 表示フォーマット: 1行目=`years`（あれば） / 2行目=`name（occupation）`（occupationが無ければ `name` のみ）
    ...
    - ベルト: 既定は `belt_mode=llm`。既存を使う場合のみ `--belt-mode existing`（belt_config.json が無い場合はスキップされる）。均等4分割は `equal`+labels、日本語必須。章構成から作る場合は `grouped`+chapters/episode_info.json。**そもそもベルト不要なら preset の `belt.enabled=false`（belt_config を無視し、テンプレ複製後に `belt*` テキストトラックを除去する）**。  
...
- CapCutドラフト生成チェックリスト（簡易）:
  ...
  4) 生成物確認: `image_cues.json` の `llm_context` に企画意図・タグが入っているか、CapCut draft が `opening_offset` 付きでシフトされているか、画像/字幕の件数がSRT行数と一致しているか。
	  5) 画像生成は `--nanobanana` で切替（`batch|direct|none`）。
	      - `batch` は Gemini Batch（Gemini系のみ）
	      - `direct` は `ImageClient(task=\"visual_image_gen\")`
	      - `none` は外部生成停止（PLACEHOLDER）
	    モデルは原則 `packages/video_pipeline/config/channel_presets.json: image_generation.model_key` でチャンネル別に固定し、未指定は tier 既定（Gemini）。
	      - CH02 は `configs/sources.yaml: channels.CH02.image_source_mix` を適用し、flux-pro:flux-max:フリー素材=7:2:1 で cue ごとに `image_model_key` を割当して生成する（この場合は `--nanobanana direct`。`batch` は単一 model_key 前提）。
	    ズレを見つけたら CSV更新→再実行（CSVは都度読み込みのため refresh は不要）。古いSRT起因のズレは `PYTHONPATH=\".:packages\" python3 -m video_pipeline.tools.align_run_dir_to_tts_final --run <run_dir>` で retime。
...
- 画像生成: セクション文脈＋企画CSVタグをプロンプトへ反映し、`nanobanana=batch|direct` で生成する（CH01は preset で Max 固定）。`image_cues.json` に企画意図・タグが入っていることを確認。
- コマンド例: `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.auto_capcut_run --channel CH01 --srt workspaces/video/input/CH01_人生の道標/220.srt --run-name jinsei220 --draft-root "$HOME/Movies/CapCut/User Data/Projects/com.lveditor.draft" --nanobanana batch --crossfade 0.5 --imgdur 20 --fps 30 --belt-mode llm`
...
- **正本（canonical）は repo 内辞書。VOICEVOX公式辞書は派生物。**
  ...
  - 全体共通の確定語（昇格済・対話型AIエージェント判定）: `packages/audio_tts/data/global_knowledge_base.json`
