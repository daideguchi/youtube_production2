...
- 内部処理の順序（auto_capcut_run.py）
  ...
  - 主要設定ポイント
    ...
    - **SRT改行整形（標準）**: 字幕の視認性のため、final SRTは改行整形済みが望ましい（SSOT: `ssot/ops/OPS_SRT_LINEBREAK_FORMAT.md`）。必要なら `python3 scripts/format_srt_linebreaks.py workspaces/audio/final/{CH}/{NNN}/{CH}-{NNN}.srt --in-place`（改行のみ、本文不変）を実行してからドラフトへ注入する。
