...
- **誤読防止（グローバル確定ルール）**:
  ...
  - 修正は「辞書/override（+必要ならAテキスト修正）」で行い、再合成する（フォールバックで続行しない）。
...
- 内部処理の順序（auto_capcut_run.py）
  ...
  - 主要設定ポイント
    ...
    - **SRT改行整形（推奨）**: 字幕の視認性のため、final SRTは改行整形済みが望ましい（SSOT: `ssot/ops/OPS_SRT_LINEBREAK_FORMAT.md`）。必要なら `python3 scripts/format_srt_linebreaks.py workspaces/audio/final/{CH}/{NNN}/{CH}-{NNN}.srt --in-place`（改行のみ、本文不変）を実行してからドラフトへ注入する。
...
- 既に混入している場合の修正順（意味を変えない最小修正）:
  1) Aテキストから **読み注釈だけ**削除（例: `刈羽郡、かりわぐんから` → `刈羽郡から`）
  2) 読みが必要なら辞書で補う（再発/一意ならCH辞書へ、曖昧ならローカルへ）
- 辞書/override の使い分け（再掲・運用の結論）:
  - **一意の読み**（どの文脈でも事故らない）→ `packages/audio_tts/data/reading_dict/CHxx.yaml` または `packages/audio_tts/configs/learning_dict.json` に昇格 → `sync_voicevox_user_dict.py` で反映
  - **曖昧語/文脈依存**（例: `行ったり` は「イッタリ/オコナッタリ」等に揺れうる）→ **辞書に入れない**。`audio_prep/local_token_overrides.json` で位置指定（必要なら語句全体で登録: `行ったり来たり` など）
