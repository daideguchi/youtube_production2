...
### 1. 音声生成準備
- `workspaces/scripts/{チャンネル}/{動画番号}/content/assembled.md` が存在することを確認
...
PYTHONPATH=".:packages" python3 packages/audio_tts/scripts/run_tts.py \
  ...
  --input "workspaces/scripts/{チャンネル}/{動画番号}/content/assembled.md" \
  --out-wav "workspaces/audio/final/{チャンネル}/{動画番号}/{チャンネル}-{動画番号}.wav" \
  --log "workspaces/audio/final/{チャンネル}/{動画番号}/log.json"
```
- 音声生成エンジン（VOICEVOX/Voicepeak/ElevenLabs）は `packages/audio_tts/tts/routing.py` が決定（必要なら `--engine-override`）。
...
- `workspaces/scripts/{チャンネル}/{動画番号}/audio_prep/` は **中間領域**（スクリプト更新時に自動 purge 対象）。下流のSoTにはしない。
...
- 前提
  ...
  - SRT は任意のパスでOK（デフォルトで `--prefer-tts-final=true` のため、episode を特定できれば `workspaces/audio/final/{CH}/{NNN}/{CH}-{NNN}.srt` を優先して使用）。
...
- 内部処理の順序（auto_capcut_run.py）
  ...
  - 主要設定ポイント
    ...
    - **CapCutテンプレ自動修復**: テンプレが `draft_info.json` のみ／空トラック名／重複トラック名でも、`tools/capcut_bulk_insert.py` が **複製直後に `draft_content.json`/`draft_info.json` を復元し、トラック名を正規化してからロード**する（レイヤ落ち・別テンプレ混入事故の根治）。監査は `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.validate_capcut_templates --active-only`。  
    ...
    - **古い/壊れたドラフト整理（推奨）**: 完成版（`★CHxx-...`）と active template 以外を CapCut root から退避する → `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.clean_capcut_channel_drafts --channel CH05 --apply`（`~/Movies/CapCut/Archive_*/CH05/` に移動）
...
- テンプレ/設定の正本:
  ...
  - `packages/video_pipeline/config/template_registry.json`: 画像プロンプトテンプレのSoT（`channel_presets.json` の `prompt_template` は登録必須 / lint: `PYTHONPATH=".:packages" python3 packages/video_pipeline/scripts/lint_check_templates.py`）。
...
- CIアーティファクト: `llm_smoke` ワークフローが実行後に LLM系ログ (`workspaces/logs/llm_usage.jsonl`, `workspaces/logs/llm_context_analyzer.log`, `workspaces/logs/tts_llm_usage.log`) をアーティファクトとして保存する（存在する場合のみ、best-effort）。
- CapCutドラフト生成チェックリスト（簡易）:
  1) `.env` が正しく読み込まれているか `python scripts/check_env.py` または `PYTHONPATH=".:packages" python3 packages/video_pipeline/check_gemini_key.py` で確認。`.venv/lib/python3.x/site-packages/factory_commentary_env.pth` により cwd を問わず sitecustomize が走る。
  ...
  5) 画像生成は `llm_context_analyzer` で得たセクション文脈＋テンプレ＋タグをそのまま Gemini image に渡すため、企画CSVを最新化しないと文脈が外れる。ズレを見つけたら CSV更新→再実行（CSVは都度読み込みのため refresh は不要）。古いSRT起因のズレは `PYTHONPATH=".:packages" python3 -m video_pipeline.tools.align_run_dir_to_tts_final --run <run_dir>` で retime。
...
### 環境変数の恒久ロード
- `.env` はリポジトリ直下に一本化。
  - Python: `sitecustomize.py` が自動読込（setdefault）。加えて `packages/audio_tts/scripts/run_tts.py` と `packages/factory_common/llm_router.py` は python-dotenv があれば `override=True` で明示ロード。
  - FastAPI（apps/ui-backend）: 起動時に repo root `.env` を読む（override=False / setdefault）。
...
- `.env` 一本化の恒久策:  
  - Python: `sitecustomize.py` が repo root `.env` を自動読込（setdefault）。加えて `packages/audio_tts/scripts/run_tts.py` と `packages/factory_common/llm_router.py` は python-dotenv があれば `override=True` でロード（事故防止）。  
  - FastAPI（apps/ui-backend）: 起動時に repo root `.env` を読込（override=False / setdefault）。  
...
- CapCutドラフト CH01 220 ロジック確認:  
  ...
  - SRT は `workspaces/video/input/CH01_人生の道標/220.srt` を指定し、`--belt-mode llm` / `--crossfade 0.5` / `--imgdur 20` / `--fps 30` がデフォルトのおすすめ。  
...
- テンプレ整理（画像プロンプト）:
  ...
  - `packages/video_pipeline/config/channel_presets.json` の `prompt_template` が参照するテンプレは **registryに登録必須**（lint: `PYTHONPATH=".:packages" python3 packages/video_pipeline/scripts/lint_check_templates.py`）。
...
- 最終音声の入力ガード:
  - 推奨: `/api/audio-tts/run-from-script` を使う（`assembled_human.md -> assembled.md -> script_sanitized.txt` の順で確定入力を自動解決）。
  - `packages/audio_tts/scripts/run_tts.py` は `assembled_human.md` が `assembled.md` より新しい場合に自動同期して「古いテキストで合成」事故を防ぐ。
