...
- 音声・字幕は `ssot/ops/OPS_AUDIO_TTS_V2.md`（現行）を参照。
...
| --- | --- | --- | --- |
| 1. リサーチ / サムネ企画 | CSV 企画案 + サムネ案 | `workspaces/planning/channels/CHxx.csv`（互換: `progress/channels/...`）, サムネフォルダ | ベンチマーク分析、サムネ手作り、CSVへの投入 |
| 2. 台本 | 詳細手順は `ssot/ops/OPS_SCRIPT_GUIDE.md` を参照（本書は手順を保持しない） | `workspaces/scripts/CHxx/<video>/content`（互換: `script_pipeline/data/...`） | - |
| 3. 音声 + SRT | WAV / SRT / Readyフラグ | `workspaces/audio/final/<channel>/<video>/`（互換: `audio_tts_v2/artifacts/final/...`） | 誤読修正、Ready承認 |
| 4. 画像+動画 | 画像一式 / 帯設定 / CapCut・Remotion draft | `workspaces/video/runs/<project>`（互換: `commentary_02_srt2images_timeline/output/...`） | 不自然な画像の差し替え、動画全体の最終確認 |
| 5. 投稿準備 | 表形式の投稿データ | `workspaces/planning/channels/CHxx.csv`（互換: `progress/channels/...`）, delivery フォルダ | チェックマーク、投稿タイミング決定 |
| 6. 自動投稿 | 投稿実行ログ | `workspaces/logs/regression/…`（互換: `logs/regression/...`）, UI からの承認ログ | 予約設定の承認 |
| 7. 分析/PDCA | アナリティクス + 改善メモ | `workspaces/planning/analytics/`（互換: `progress/analytics/...`）, `ssot/history/HISTORY_codex-memory.md` | 指標の読み取り、次企画への指示 |

## 3. フェーズ別要件

### フェーズ1：チャンネル起ち上げ / サムネ体系化
- ベンチマーク分析で「誰に刺さるか / サムネ構成 / タイトル / 尺」の勝ちパターンを洗い出す。
- 企画 CSV をチャンネルごとに作成（1行1動画）。SoT は `workspaces/planning/channels/CHxx.csv`（互換: `progress/channels/...`。master channels CSV は廃止）。
- サムネイルは管理者が手動で作り、チャンネルごとのフォルダに保存。**企画とサムネが必ず紐づいて確認できる UI / フォルダ構造**を維持。

### フェーズ2：台本・音声ラインの自動化
- 台本工程: `ssot/ops/OPS_SCRIPT_GUIDE.md` を唯一の参照とする。本書に手順は記載しない。
- 音声/字幕: `ssot/ops/OPS_AUDIO_TTS_V2.md` を参照。
- 企画 CSV はチャンネル別 SoT（`workspaces/planning/channels/CHxx.csv`。互換: `progress/channels/...`）。
...
| 音声/SRT | 自動合成と自動チャンクを標準（音声ラインは `audio_tts_v2`） | 読みやすさの最終判断、Ready承認 |
...
## 5. データ管理・SoT
- 企画: `workspaces/planning/channels/CHxx.csv`（チャンネル別が正本。互換: `progress/channels/...`。master channels CSV は使用しない）
- 台本A/B: `workspaces/scripts/CHxx/<video>/content`（互換: `script_pipeline/data/...`） / 音声: `workspaces/audio/final/<channel>/<video>/`（互換: `audio_tts_v2/artifacts/final/...`）
- 画像・帯・動画ドラフト: `workspaces/video/runs/<project>/`（互換: `commentary_02_srt2images_timeline/output/...`）
- 投稿成果: `delivery/CHxx/CHxx-###/`
- サムネ: `thumbnails/<channel>/<...>`（フォルダ構成で管理）
- 投稿後指標/改善メモ: `workspaces/planning/analytics/<channel>.csv`（互換: `progress/analytics/...`。アナリティクスの SoT として運用する）

## 6. PDCA とログ
- すべての工程でログを残し、`ssot/history/HISTORY_codex-memory.md` に記録。  
- UI/CLI で実行したガード（`progress_manager validate-status`, `workflow_precheck` など）はログディレクトリへ保存し、異常があれば即時に差し戻す。  
- Ready queue / 投稿 / 分析結果は CSV + 履歴ログの双方に記録し、次フェーズへのインプットにする。

### 6.1 サムネイル承認フロー（人間の責務）
1. **候補作成**: 管理者がサムネ画像を作成し、`thumbnails/<channel>/` に配置。各案には意図や KPI をメモする。
...
3. **ログ保存**: 投稿実行ログ (`workspaces/logs/regression/*`。互換: `logs/regression/*`) と承認ログ（CSV or HISTORY）を紐付けておき、後で「誰がいつ承認したか」を追跡できる状態にする。
...
flowchart LR
    ...
    B --> C[TTS生成\naudio_tts_v2 → artifacts/final/<ch>/<vid>]
    C --> D[画像/帯生成\ncommentary_02]
...
- 音声成果物の公式保存先: `workspaces/audio/final/<channel>/<video>/`（互換: `audio_tts_v2/artifacts/final/...`）。  
...
| T2 | A/B台本整備と音声Readyフラグ | ✅ 完了（OPS_AUDIO_TTS_V2） |
...
## 9. API / CLI 依存関係（主要）
- `/api/planning/*` ↔ `progress_manager` ↔ `workspaces/planning/channels/CHxx.csv`（互換: `progress/channels/...`）
- `/api/channels/{code}/profile` ↔ `channel_profile`, `workspaces/planning/personas`（互換: `progress/personas`）
- `/api/video-production/*` ↔ `commentary_02` CLI (`generate_belt_layers.py`, `capcut_bulk_insert.py`)
- `/api/ssot/persona/{channel}` ↔ `workspaces/planning/personas/CHxx_PERSONA.md`（互換: `progress/personas/...`）
- 台本ステージは `PYTHONPATH=. python -m script_pipeline.cli run-all --channel CHxx --video NNN` を唯一の入口とし、CLI が `status.json` と `workspaces/planning/channels/CHxx.csv`（互換: `progress/channels/...`）を同期する。手動更新は禁止。
...
6. **分析で終わり分析に戻る**：`workspaces/planning/analytics/<channel>.csv`（互換: `progress/analytics/...`）を必ず更新し、次の企画に反映する。
