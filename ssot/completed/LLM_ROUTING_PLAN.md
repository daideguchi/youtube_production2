...
- **TTS タスクマップ**: `audio_tts_v2/configs/llm_tasks.yaml` で annotate/reading など TTS サブタスクを `tts_primary` に束ね、呼び出しは `LLMRouter/LLMClient` に移行済み（旧 `llm_client` は廃止予定）。【F:audio_tts_v2/configs/llm_tasks.yaml†L1-L7】【F:audio_tts_v2/tts/llm_adapter.py†L1-L120】
...
- 現在は `LLMRouter/LLMClient` を使用し、task→tier→model で解決（`llm_client.py` は段階的削除予定）。TTS側の呼び出しは `audio_tts_v2/tts/llm_adapter.py` が集約。
