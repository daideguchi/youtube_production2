...
## 0. TL;DR（ざっくり要約）
- **どこを見ればいい？** 表は「どのステージで何トークンくらい使い、どの tier/model が最適か」を一目で示す。迷ったら `2. ステージ別トークン予測とモデル選定` を見る。
- **総コスト感**: 台本は長尺化（例: CH10は 15k+ 字）で増える。基本は「DeepSeek V3.2 Exp（thinking有効）」を主にし、長文の安定化/最終磨き込みは Kimi K2 Thinking を使う。R1 系は使わない（8k 上限と挙動差で事故りやすい）。
- **優先アクション（短期）**:
...
## 2. ステージ別トークン予測とモデル選定（更新版）
### 2.1 台本パイプライン（script_pipeline）
| ステージ | 入力ボリューム (概算) | 期待出力 | 1 回あたり推定 Tokens (prompt + completion) | 呼び出し回数 | モデル/ tier | 根拠 |
| --- | --- | --- | --- | --- | --- | --- |
...
| script_validation（LLM品質） | assembled A-text | Judge JSON / Fix / 最終ポリッシュ | 中〜高 | 1〜数回 | Judge/Fix: **or_deepseek_v3_2_exp** → fallback **or_kimi_k2_thinking** / 最終ポリッシュ: **or_kimi_k2_thinking** | 「冗長/反復/概念混線/締め未完」を確実に落とし、不足時は1回で磨く。 |

**累計（概算）**（7 章想定）: 章数と目標字数に比例して増える。長尺（15k+字）では `script_validation` が追加1回（最終ポリッシュ）で増えるが、往復が減れば総コストは下がりやすい。

### 2.2 B テキスト/TTS（audio_tts/tts）
| ステージ | 入力ボリューム | 期待出力 | 推定 Tokens | 呼び出し回数 | モデル/ tier | 根拠 |
| --- | --- | --- | --- | --- | --- | --- |
...
**累計（概算）**: ~47k tokens。script と合わせても 128k 以内だが、B テキスト生成は別ジョブで実行する。
