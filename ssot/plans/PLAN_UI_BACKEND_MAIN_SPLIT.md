...
| 3e | `routers/thumbnails_video.py` の `ssot_assets_root`/`ssot_thumbnails_root` import を修正（`assets_root`/`thumbnails_root` を alias で使用）し、UI-backend 起動時エラーを解消（挙動互換） | dd | - | Draft |
