...
| 2by | Thumbnails library models（library asset/rename/import/assign/history）+ extensions 定数を `app/` に移設し、`routers/thumbnails_workspace.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
