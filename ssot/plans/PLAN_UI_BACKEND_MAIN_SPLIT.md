...
| 2bz | Thumbnails workspace models（`ThumbnailProjectUpdateRequest`, `ThumbnailDescriptionResponse`）を `app/` に移設し、`routers/thumbnails_workspace.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
