...
| 2cc | Thumbnails editor/preview models（editor context / preview / comment patch + schema const）を `app/` に移設し、`routers/thumbnails_video.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
