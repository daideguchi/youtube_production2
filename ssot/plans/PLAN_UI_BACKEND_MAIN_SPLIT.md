...
| 2bb | Planning routers（`routers/planning_csv.py`, `routers/planning_channel.py`, `routers/video_planning.py`）の入力正規化（`normalize_*`）を `app/normalize.py` に寄せ、`backend.main` 依存を削減（挙動互換） | dd | - | Draft |
