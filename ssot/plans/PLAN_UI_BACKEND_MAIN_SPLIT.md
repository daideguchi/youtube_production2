...
| 4u | LLM settings handlers（`get_llm_settings`, `update_llm_settings`, `_build_llm_settings_response`）を `app/llm_settings_handlers.py` に移設し、`main.py` は import に寄せる（挙動互換） | dd | - | Draft |
