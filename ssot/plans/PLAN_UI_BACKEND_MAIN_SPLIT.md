...
| 4as | Channel videos router（`routers/channel_videos.py`）の `backend.main` runtime import（`_collect_disk_thumbnail_variants`）を `app/thumbnails_disk_variants.py` へ移設し、循環import/404リスクを下げる（挙動互換） | dd | - | Draft |
