...
| 4n | Audio metadata path normalize（`normalize_audio_path_string`, `normalize_audio_metadata`）を `app/audio_metadata_utils.py` に移設し、`main.py` は import に寄せる（挙動互換） | dd | - | Draft |
