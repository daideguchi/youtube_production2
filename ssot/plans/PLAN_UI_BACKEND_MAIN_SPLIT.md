...
| 4ao | Audio review router（`routers/audio_review.py`）の残り `backend.main` 依存（`summarize_log`, `_resolve_channel_title`）を `backend/app` へ移設し、runtime import を解消（挙動互換） | dd | - | Draft |
