...
| 4ae | TTS plain router（`routers/tts_text.py`）の `backend.main` 依存（`audio_final_dir`, `video_base_dir`, `resolve_text_file`, 正規化）を外し、`backend/app` + `factory_common.paths` へ寄せる（挙動互換） | dd | - | Draft |
