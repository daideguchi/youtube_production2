...
| 4aq | Effective stages/status（`_derive_effective_stages`, `_derive_effective_video_status` + artifact-based injectors）を `app/video_effective_status.py` に移設し、dashboard/channel_videos の `backend.main` runtime import を削減（挙動互換） | dd | - | Draft |
