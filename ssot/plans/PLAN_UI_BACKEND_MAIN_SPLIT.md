...
| 4ap | Audio history helper（`append_audio_history_entry`）を `app/status_store.py` へ移設し、`routers/channel_videos.py` / `routers/tts.py` の `backend.main` runtime import を削減（挙動互換） | dd | - | Draft |
