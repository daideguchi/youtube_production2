...
| 4f | Video channel presets store（`channel_presets.json` の I/O: `VIDEO_CHANNEL_PRESETS_LOCK`, `_load_video_channel_presets_document`, `_write_video_channel_presets_document`）を `app/video_channel_presets_store.py` に移設し、`main.py` は import に寄せる（挙動互換） | dd | - | Draft |
