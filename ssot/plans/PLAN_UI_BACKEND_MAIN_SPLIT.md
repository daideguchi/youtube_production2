...
| 2cf | Thumbnails templates store（`THUMBNAIL_TEMPLATES_LOCK`, `_load_thumbnail_templates_document`, `_write_thumbnail_templates_document`）を `app/thumbnails_templates_store.py` に移設し、`routers/thumbnails_templates.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
