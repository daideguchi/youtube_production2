...
| 2ca | Thumbnails video request models（variant create/generate/compose/patch + asset replace response）を `app/` に移設し、`routers/thumbnails_video.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
