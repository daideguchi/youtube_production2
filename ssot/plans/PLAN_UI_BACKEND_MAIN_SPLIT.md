...
| 2cd | Thumbnails layer-spec models（layer-spec responses + image model info + param catalog entry）を `app/` に移設し、`routers/thumbnails_specs.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
