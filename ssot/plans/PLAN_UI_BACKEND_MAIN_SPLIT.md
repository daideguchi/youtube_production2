...
| 2cc | Thumbnails editor/preview models（editor context / preview / comment patch + schema const）を `app/` に移設し、`routers/thumbnails_video.py` の `backend.main` 依存を削減（挙動互換） | dd | - | Done（2026-01-14） |
...
| 2cg | Thumbnails projects store（`THUMBNAIL_PROJECTS_LOCK`, `_load_thumbnail_projects_document`, `_write_thumbnail_projects_document`）を `app/thumbnails_projects_store.py` に移設し、thumbnail系 router の `backend.main` 依存を削減（挙動互換） | dd | - | Done（2026-01-14） |
...
| 2cl | Routers の `ensure_expected_updated_at` 参照を `backend.main` → `app/status_models.py` に切替し、`backend.main` 依存を段階的に削減（挙動互換） | dd | - | Done（2026-01-14） |
...
| 4y | Human scripts router（`routers/human_scripts.py`）の `backend.main` 依存を外し、status/store/lock は `backend/app` へ寄せる（挙動互換） | dd | - | Done（2026-01-15） |
| 4z | Redo flags router（`routers/redo_flags.py`）の `backend.main` 依存を外し、`app/status_store.py` の `save_status` に寄せる（挙動互換） | dd | - | Done（2026-01-15） |
...
| 4an | Settings router（`routers/settings.py`）の `backend.main` runtime import（`get_llm_settings`, `update_llm_settings`, `get_codex_settings`, `update_codex_settings`）を外し、`app/llm_settings_handlers.py` + `app/codex_settings_store.py` に寄せる（挙動互換） | dd | - | Done（2026-01-15） |
...
| 4az | Thumbnail stable spec helpers（`_normalize_thumbnail_stable_id`, `_thumb_spec_stable_path`, `_text_line_spec_stable_path`, `_elements_spec_stable_path`）を `app/thumbnails_stable_paths.py` に移設し、thumbnail系 router の `backend.main` 依存を削減（挙動互換） | dd | - | Draft |
