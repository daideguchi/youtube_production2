...
| 4aj | Planning channel router（`routers/planning_channel.py`）の module-level `backend.main` import を外し、`backend/app` + lazy import へ寄せて循環import/404リスクを下げる（挙動互換） | dd | - | Draft |
