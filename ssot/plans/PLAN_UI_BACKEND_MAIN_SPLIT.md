...
| 4ah | Channel videos router（`routers/channel_videos.py`）の module-level `backend.main` import を外し、`backend/app` + lazy import に寄せて循環import/404リスクを下げる（挙動互換） | dd | - | Draft |
