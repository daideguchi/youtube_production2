...
- 台本（デフォルト）: `./ops script new -- --channel CHxx --video NNN`（既存なら `resume`）
  ...
  - 明示CLI（Claude; 既定）: `./ops claude script -- --channel CHxx --video NNN --run`（Blueprint必須。sonnet 4.5。opusは **オーナー指示 + `YTM_ALLOW_CLAUDE_OPUS=1` の明示実行のみ**。リミット時は Gemini 3 Flash Preview → qwen）
