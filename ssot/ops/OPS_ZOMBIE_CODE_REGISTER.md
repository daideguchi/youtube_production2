...
- 時点情報（必須）: “いつのロジックか” を必ず残す（created/updated）。
  - scripts: `ssot/ops/OPS_SCRIPTS_INVENTORY.md` の `created/updated` が正本
  - config/code: `git log --follow` の `created/updated` を根拠として明記する（推測しない）
...
| path | created | updated | 備考 |
| --- | --- | --- | --- |
| `configs/llm.yml` | 2025-12-11 | 2026-01-09 | ファイル先頭に “当時のモデル選定の時点情報” コメントあり |
| `packages/factory_common/llm_config.py` | 2025-12-13 | 2026-01-09 | legacy互換の設定ローダ |
| `packages/factory_common/llm_client.py` | 2025-12-13 | 2025-12-26 | legacy互換のクライアント |
...
| YML系（legacy） | `configs/llm.yml`, `configs/llm.local.yml`, `packages/factory_common/llm_client.py`, `packages/factory_common/llm_config.py` | 互換/テスト用（通常運用はロックダウンで停止） | “どれが正本か”が崩れる（迷子/誤実行の温床） |
...
2) LLM legacy（`configs/llm.yml` / `factory_common.llm_client` / `factory_common.llm_config`）をいつ archive-first→delete するか？（通常運用は既にロックダウンで禁止）
