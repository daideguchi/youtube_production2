...
- `logs/agent_tasks/{pending,results,completed}/*.json`  
  ...
  - 関連:
    ...
    - `logs/agent_tasks/coordination/orchestrator/*`（orchestrator state/inbox/outbox）
      - Writer/Reader: `scripts/agent_org.py`
  - 備考: `logs/` は gitignore のため増えやすい。不要になった結果は `ssot/plans/PLAN_OPS_ARTIFACT_LIFECYCLE.md` に従い退避/削除。
  - 種別: **L1（運用SoT）**
...
- 2025-12-12: `commentary_02_srt2images_timeline/{src,ui/src}/memory/**` は参照ゼロの確実ゴミとして削除済み（`ssot/ops/OPS_CLEANUP_EXECUTION_LOG.md`）。
