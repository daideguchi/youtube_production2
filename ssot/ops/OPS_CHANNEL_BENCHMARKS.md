...
既存チャンネルの `youtube_description / default_tags / benchmarks` を **一括でシード**し、`channels_info.json` を再生成する。

- dry-run（書き込みなし）: `python3 scripts/ops/channel_info_normalize.py`
- 適用（書き込み + カタログ再生成）: `python3 scripts/ops/channel_info_normalize.py --apply`
- 1チャンネルだけ: `python3 scripts/ops/channel_info_normalize.py --channel CH07 --apply`
