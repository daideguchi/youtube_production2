...
## 0) 重要: LLMゲートは“定性レビュー”の代替ではない

LLM Judge/Fixer は「最低品質を落とさない」ための安全弁だが、  \n
**最終的に“人間が聞いて自然か”の判断は置き換えられない**。

運用ルール（必須）:
- 台本の生成/修正後は、`ssot/ops/OPS_A_TEXT_GLOBAL_RULES.md` の **通し読みの定性レビュー**を先に行う。
- 【最優先】日本語はネイティブが聞いて自然な口語にする（翻訳調/報告書調/直訳調は禁止）。
  - NGの匂い: 「〜において/〜に関して/〜によって」「〜することが可能」「〜が示唆される」「〜である」「〜と考えられる」の連発
  - OKの方向: 短い文で言い切る。主語と述語を近づける。抽象で逃げず、具体でつなぐ。
- LLM Judge の `pass` は “形式・構造の合格” に近い。  \n
  不自然な言い回し/比喩/流れのギクシャクが残っていれば、書き手が手で直してから次工程へ進む。
- 目標: 「ツールを回してから破綻に気づきやり直し」を繰り返さない。

## 1) 基本方針（LLM + ハードガードで品質を固定）

### 1.0 ハードガード（機械チェック/非LLM）を先に通す
- `script_validation` はまず機械チェック（非LLM）で「禁則/字数/区切り/括弧上限」等を検査し、必要時は **TTS事故を防ぐための形式レベルの最小差分**（例: 行内 `---` の正規化、メタ混入の除去）だけを行ってから LLM Judge に渡す。
...
### 1.0.1 Judge の“数え間違い”だけを補正する（オプション; 安全弁）
LLM Judge は「内容品質」を見るのが役割だが、**数え間違い**（`---` 本数、`「」/（）` の個数、人物例の数など）を起こすことがある。  
ここで誤って `must_fix` になり Fix ループが回るとコストが跳ねるため、`script_validation` では **客観的に検証できる“数”だけ**を（オプションで）補正できる。

- 既定: ON: `SCRIPT_VALIDATION_PRUNE_JUDGE_OBJECTIVE_MISCOUNTS=1`
  - `---` 本数（pause_lines）
  - `「」`/`（）` の上限（tts_hazard）
  - “人物例” の数え間違い（modern_examples_count）
- 既定: OFF（デバッグ専用）: `SCRIPT_VALIDATION_PRUNE_JUDGE_QUALITATIVE_MUST_FIX=1`
...
### 1.2.1 Final Polish（最終納品の磨き込み / オプション）
目的:
...
- 例外:
  ...
  - 無効化（常に実行したい）: `SCRIPT_VALIDATION_LLM_MAX_A_TEXT_CHARS=0`（注意: コスト/コンテキスト事故の原因になるため常用しない）
...
- “リトライで押し切る” ではなく、**最初のJudgeで本質的な欠陥だけを抽出し、Fixは最大1回（追加は+1回まで）で収束させる**。

運用調整:
- `SCRIPT_VALIDATION_LLM_MAX_ROUNDS` で上書き可能（ただし hard cap: 通常=3, `codex_exec`=5）
- コストを優先して短く止めたい場合は `SCRIPT_VALIDATION_LLM_MAX_ROUNDS=2` に下げる（手順: まずは既定で運用して失敗率を見てから）。
...
## 3) Fixer の修正ルール（品質と安全）

- **核の維持**: 既存台本の「伝えたいこと」と中心エピソードは保持する（ただし、核自体が弱い場合は Judge 指示に従い組み替え可）。
