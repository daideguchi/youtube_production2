...
### 1.0 ハードガード（決定論）を先に通す
- `script_validation` はまず決定論で「禁則/字数/区切り/括弧上限」等を検査し、必要なら **最小差分**で修正してから LLM Judge に渡す。
...
- 収束の実装方針（現行）:
  ...
  - それでも `length_too_long` が残る場合、最後の手段として **決定論の「区切り（`---`）単位の予算トリム」**で目標レンジに収める（`---` の位置と個数は維持）。
