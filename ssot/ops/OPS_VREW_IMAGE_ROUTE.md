...
### 7.4 schnell向けテクニック（取り込み方）

VrewのAI画像生成が `FLUX.1 schnell` 系を含む想定で、**「狙い通りの構図」** を出しやすくするための方針を以下に固定する。

- 基本方針:
  - 1行=1画像の意図（句点で分割されるため、本文内に余計な `。` は入れない）
  - **撮影指示書レベルで具体化**（誰が、どこで、何をしている、構図、光）
  - 形容詞の盛り過ぎは避け、位置や情報量など **具体パラメータ** を優先する
- `style_prefix`（共通の固定部分）に入れるもの:
  - 画風、人物の統一条件（年齢感や家族構成など）、舞台の質感、光の傾向、色の傾向
  - 例: 二次元の日本イラスト、柔らかい自然光、落ち着いた色調、生活感
- `constraints`（毎行に効かせる固定ルール）に入れるもの:
  - 構図の安定化（中景、三分割、主役位置、背景の情報量、余白の意図）
  - 破綻回避（手や指を強調しない、不要な人物を増やさない）
  - 例: 十六対九、中景、主役は右寄り、左に余白、背景は単純、文字なし
- seed/stepsの扱い:
  - Vrew側でseed固定ができない場合は、同一文のまま再生成して当たりを引く（ガチャ前提）
  - seed固定が可能なら、当たりのseedを固定して不足部分だけを追記して詰める

補足:
- 生成本文はUI運用上「日本語のプレーンテキスト」を優先し、英字や装飾記号はサニタイズで除去する（SSOT準拠）。
