...
- 主キー（重複判定の軸）: `キーコンセプト`
- 補助タグ: `悩みタグ_メイン`, `悩みタグ_サブ`（追加で `ライフシーン` を併用してよい）

運用ルール:
- `キーコンセプト` は「その回の核」を短い語で固定し、同一チャンネル内で重複を避ける（完全禁止ではない）。
...
- `内容（企画要約）`
  - タイトル先頭 `【...】` と、要約先頭 `【...】` が不一致なら **別テーマ混入**の可能性が高いので、台本生成では無視する（CSVを修正する）。
...
※ UI/運用上の表示・編集も「タイトル / 補助 / 禁止」を分けるのが安全（本文っぽいサンプルは既定で非表示）。
...
- Planning CSV の `進捗=投稿済み` は **公開済みロック**（以後は触らない）。
