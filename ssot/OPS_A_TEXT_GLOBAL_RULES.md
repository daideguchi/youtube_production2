...
- 運用上の扱い: この「目安」は **script_validation のハード上限**として扱う（超過は不合格）。
  ...
  - 超過した場合は **決定論クリーナで括弧だけ外して基準内に落とす**（意味は維持・全文リライト禁止）。
...
- 運用の正本: `ssot/OPS_A_TEXT_LLM_QUALITY_GATE.md`（Judge→Fixer の2段階で最終品質を固定）。
...
補足:
- チャンネル `script_prompt.txt` に構成/形式/記号の指示が混ざっていても、Aテキスト向けLLM呼び出しでは衝突しやすい要素を落とした `a_text_channel_prompt`（派生）を用いる。骨格は `ssot/OPS_SCRIPT_PATTERNS.yaml` が正本。
