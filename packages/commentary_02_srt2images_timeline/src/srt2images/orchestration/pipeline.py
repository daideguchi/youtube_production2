...
def run_pipeline(args):
    ...
    def select_image_model_for_channel(channel: str) -> str:
        ...
        基本方針:
          - CH01: gemini-2.5-flash-image (429対策のため一時的に2.5へダウングレード)
          - それ以外: gemini-2.5-flash-image
        """
        channel_upper = (channel or "").upper()
        if channel_upper == "CH01":
            # return "gemini-3-pro-image-preview" # 429 Resource Exhaustedのため無効化
    ...
    # チャンネルに応じて画像モデルを設定
    image_model = select_image_model_for_channel(args.channel or detected_channel)
    os.environ["SRT2IMAGES_IMAGE_MODEL"] = image_model
    os.environ.pop("SRT2IMAGES_MEDIA_RESOLUTION", None)

    logging.info(
        "%s detected: using Gemini image model override=%s",
        channel_upper or "Unknown channel",
        os.environ.get("SRT2IMAGES_IMAGE_MODEL"),
    )
