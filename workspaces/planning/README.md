...
- 正本: `channels/CHxx.csv`（UI と `planning_store` が直接参照）。手動編集ルールは `ssot/OPS_PLANNING_CSV_WORKFLOW.md` に従う。
- Script の進捗 SoT は `script_pipeline/data/CHxx/NNN/status.json`（このディレクトリではない）。
