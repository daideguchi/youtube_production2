...
- 固定ルール: `script_*`（台本）は agent/think-mode 対象外（Codex/agent 代行で台本を書かない）。台本は常に `LLM_EXEC_SLOT=0`（API）で実行する。
...
- [`RUNBOOK_JOB_SCRIPT_PIPELINE.md`](/ssot/agent_runbooks/RUNBOOK_JOB_SCRIPT_PIPELINE.md): 台本生成（end-to-end / API固定）
...
## Legacy（参照用）
- [`RUNBOOK_SCRIPT_WRITING.md`](/ssot/agent_runbooks/RUNBOOK_SCRIPT_WRITING.md): 旧・台本生成（agent/think-mode は廃止済み）
