...
- `_archive/`: 旧サムネ資産・未採用案などの退避先（gitignore）。Top階層をクリーンに保つため、古い資産はここに移動します。
- `ui/`: UI 実装メモやコンポーネント設計など、フロントエンド側の補助資料を配置。
- `automation/`（任意）：サムネイル自動生成のためのスクリプトを追加していく想定。

注: `CHxx_.../` や `CHxx-.../` のような旧形式のディレクトリは Legacy（現行UI/コード非参照）です。必要なら `_archive/<timestamp>/` に退避し、トップ階層には置かないでください。
